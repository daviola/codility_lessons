...
# First solution 62%
def solution_alt(S, P, Q):
    impact_weight = {'A': 1, 'C':2, 'G': 3, 'T':4}    
    impacts = []
    for i in range(len(P)):
        impact = 5
        for elem in S[P[i]:Q[i]+1]:            
            if impact > impact_weight[elem]:
                impact = impact_weight[elem]
...
def solution(S, P, Q):
    ...
    impacts = []        
    for i in range(len(P)):
        for impact in impact_weight:
            try:
                S[P[i]:Q[i]+1].index(impact)
                impacts.append(impact_weight[impact])
                break
            except:
                continue
    return impacts    